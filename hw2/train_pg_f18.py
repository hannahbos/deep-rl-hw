...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "policy_discrete",
                self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, "policy_continues", self.n_layers,
                size=self.size)
            # this variable is created here
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            dist = tfp.distributions.Multinomial(total_count=1.0, logits=sy_logits_na)
            sy_sampled_ac = dist.sample()
            # select action
            sy_sampled_ac = tf.argmax(sy_sampled_ac, axis=-1)
        else:
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            q_n = []
            for i,rew in enumerate(re_n):
                discounted_rew = rew*np.power(self.gamma,np.arange(len(rew)))
                summed_rew = np.cumsum(discounted_rew[::-1])[::-1]
                q_n = np.concatenate([q_n, summed_rew])
        else:
            q_n = []
            for i,rew in enumerate(re_n):
                # discount starts at time step zero
                discounted_rew = rew*np.power(self.gamma,np.arange(len(rew)))
                # all time points in one trajectory get multiplied by the same
                # summed reward factor
                summed_rew = np.sum(discounted_rew)*np.ones_like(rew)
                q_n = np.concatenate([q_n, summed_rew])
        return q_n
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            if std_adv > 0:
                adv_n = (adv_n - np.mean(adv_n))/std_adv
            else:
